...
void FbxLoader::ParseMesh(FbxModel* fbxModel, FbxNode* fbxNode)
{
	...
	//頂点座標読み取り
	ParseMashVertices(fbxModel, fbxMesh);
	//面を構築するデータの読み取り
	ParseMashFaces(fbxModel, fbxMesh);
	//マテリアル読み取り
...
void FbxLoader::ParseMashVertices(FbxModel* fbxModel, FbxMesh* fbxMesh)
{
...
void FbxLoader::ParseMashFaces(FbxModel* fbxModel, FbxMesh* fbxMesh)
{
	auto& vertices = fbxModel->vertices;
	...
	for (int i = 0; i < vertices.size(); i++) {
		...
		for (auto& weightSet : weightList) {
			...
			if (++weightArrayIndex >= FbxModel::MAX_BONE_INDICES) {
				float weight = 0.0f;
				//2番目以降のウェイトを合計
				for (int j = i; j < FbxModel::MAX_BONE_INDICES; j++) {
